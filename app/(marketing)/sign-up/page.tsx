...
export default function SignUpPage() {
  ...
  // Mock handling the Google Sign-in click
  const handleGoogleSignIn = () => {
    if (!role) {
      alert("please select a role first!");
      return;
    }
    // In a real app, we'd call signIn("google") here.
    // For now, we simulate success and move to the onboarding step.
    setStep(2);
  };
        ...
        <div className="max-w-md w-full transition-all duration-500">
          ...
          {step === 1 ? (
            ...
            <div className="animate-in fade-in slide-in-from-right-8 duration-700 fill-mode-both">
              <button onClick={() => setStep(1)} className="mb-8 text-outline-variant hover:text-brand-teal flex items-center gap-1 transition-colors text-sm font-semibold">
                <span className="material-symbols-outlined text-lg">arrow_back</span> back
              </button>
              
              <div className="mb-10 text-center lg:text-left">
                <h2 className="text-4xl font-extrabold font-headline text-on-surface tracking-tight mb-2">almost there</h2>
                <p className="text-on-surface-variant font-body">you signed in via google. let's finalize your {role} profile.</p>
              </div>
              
              <form onSubmit={handleCompleteProfile} className="space-y-6">
                <div className="space-y-2">
                  <label className="font-label text-xs font-semibold text-on-surface-variant ml-1">preferred name</label>
                  <input required className="w-full px-5 py-4 bg-surface-container-low border border-transparent rounded-full focus:outline-none focus:border-brand-teal focus:ring-1 focus:ring-brand-teal transition-all font-body text-on-surface placeholder:text-outline-variant" placeholder="what should we call you?" type="text" />
                </div>
                ...
                <button className="w-full bg-gradient-to-br from-[#006064] to-[#007b80] text-white font-bold py-4 rounded-full shadow-lg shadow-brand-teal/20 hover:opacity-90 active:scale-[0.98] transition-all flex justify-center items-center gap-2 mt-4 focus:outline-none focus:ring-1 focus:ring-brand-teal focus:ring-offset-1" type="submit">
